...
2. Tag that commit with `v` followed by the exact version (example: `v0.1.1`). Push the tag.
...
```sh
gh workflow run release.yml --repo casadi/torch2casadi --ref v0.1.1 -f target=testpypi
```
...
```sh
gh attestation verify torch2casadi-0.1.1-py3-none-any.whl --repo casadi/torch2casadi
```
